...
uint16_t FT6336U::read_touch2_x(void) {
    uint8_t read_buf[2];
    read_buf[0] = readByte(FT6336U_ADDR_TOUCH2_X + 6);
    read_buf[1] = readByte(FT6336U_ADDR_TOUCH2_X + 1 + 6);
	return ((read_buf[0] & 0x0f) << 8) | read_buf[1];
}
uint16_t FT6336U::read_touch2_y(void) {
    uint8_t read_buf[2];
    read_buf[0] = readByte(FT6336U_ADDR_TOUCH2_Y + 6 );
    read_buf[1] = readByte(FT6336U_ADDR_TOUCH2_Y + 1 + 6);
	return ((read_buf[0] & 0x0f) << 8) | read_buf[1];
}
